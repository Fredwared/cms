...
<div class="box box-primary">
	<!-- form start -->
	<form id="frmRole" name="frmRole" role="form" action="{{ route('backend.role.store') }}" method="post">
		<div class="box-body">
			...
			<div class="row">
                @if (count(config('cms.backend.languages')) == 1)
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-group">
                        <input type="text" class="form-control" id="role_name_{!! $key !!}" name="role_name[{!! $key !!}]" value="{{ old('role_name.' . $key) }}" placeholder="Tên quyền">
                    </div>
                @else
                    @foreach (config('cms.backend.languages') as $key => $lang)
                        <div class="col-lg-{{ 12 / count(config('cms.backend.languages')) }} col-md-{{ 12 / count(config('cms.backend.languages')) }} col-sm-12 col-xs-12 form-group has-feedback">
                            <input type="text" class="form-control" id="role_name_{!! $key !!}" name="role_name[{!! $key !!}]" value="{{ old('role_name.' . $key) }}" placeholder="{{ $lang['native'] }}">
                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 form-group">
                    ...
                    <input type="text" class="form-control" id="role_code" name="role_code" value="{{ old('role_code') }}" placeholder="Mã quyền">
                </div>
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 form-group">
                    <label for="role_priority">Độ ưu tiên</label>
                    <input type="text" class="form-control" id="role_priority" name="role_priority" value="{{ old('role_priority') }}" placeholder="Độ ưu tiên">
                </div>
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 form-group">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-group">
                    ...
                    <select class="form-control" id="action_applied" name="action_applied[]" data-multiselect="true" data-tags="true" data-width="100%" data-placeholder="Chọn action"  multiple="multiple">
                        @foreach ($actions as $action)
                            <option value="{{ $action }}"{!! in_array($action, old('action_applied', [])) ? ' selected="selected"' : '' !!}>{{ $action }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
