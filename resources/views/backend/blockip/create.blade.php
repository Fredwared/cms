...
<div class="box box-primary">
	<!-- form start -->
	<form id="frmBlockIp" name="frmBlockIp" role="form" action="{{ route('backend.blockip.store') }}" method="post">
		<div class="box-body">
			<div class="row">
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 form-group">
                    <label for="ip_address" class="required">Địa chỉ ip</label>
                    <input type="text" class="form-control" id="ip_address" name="ip_address" value="{{ old('ip_address') }}" placeholder="Địa chỉ ip">
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 form-group">
            ...
            <div class="row">
                @if (count(config('cms.backend.languages')) == 1)
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-group">
                        <input type="text" class="form-control" id="ip_description_{!! $key !!}" name="ip_description[{!! $key !!}]" value="{{ old('ip_description.' . $key) }}" placeholder="Mô tả">
                    </div>
                @else
                    @foreach (config('cms.backend.languages') as $key => $lang)
                        <div class="col-lg-{{ 12 / count(config('cms.backend.languages')) }} col-md-{{ 12 / count(config('cms.backend.languages')) }} col-sm-12 col-xs-12 form-group has-feedback">
                            <input type="text" class="form-control" id="ip_description_{!! $key !!}" name="ip_description[{!! $key !!}]" value="{{ old('ip_description.' . $key) }}" placeholder="{{ $lang['native'] }}">
