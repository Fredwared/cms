        ...
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                @if (check_permission('buildtop', 'update'))
                    <button type="button" class="btn btn-sm btn-primary" id="btnSave" data-link="{{ route('backend.article.buildtop.save') }}"><i class="fa fa-save"></i> Lưu</button>
                @endif
                @if (check_permission('buildtop', 'delete'))
                    <button type="button" class="btn btn-sm btn-primary" id="btnDelete" data-link="{{ route('backend.article.buildtop.destroy') }}"><i class="fa fa-trash"></i> Xóa</button>
                @endif
                <div class="table-responsive">
                    <table class="table table-hover" id="tblListBuildtop" data-link="{{ route('backend.article.buildtop.index') }}">
                        <thead>
                            <tr>
                                <th class="w10px">
                        ...
                        <tbody>
                            @include('backend.article.buildtop.listbuildtop')
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                ...
                @endif
                <div class="table-responsive">
                    <table class="table table-hover" id="tblListSearch" data-link="{{ route('backend.article.buildtop.listarticle') }}">
                        <thead>
                            <tr>
                                <th class="w10px">
                                    <input type="checkbox" class="checkbox" id="chkAll" />
