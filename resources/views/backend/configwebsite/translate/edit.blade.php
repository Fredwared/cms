...
<div class="sidebar-content sidebar-md">
    <form id="frmTranslate" name="frmTranslate" role="form" action="{{ route('backend.configwebsite.translate.update', [$translateInfo->translate_id]) }}" method="put">
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 form-group">
                <label for="translate_code" class="required">Code</label>
                <input type="text" class="form-control" id="translate_code" name="translate_code" value="{{ old('translate_code', $translateInfo->translate_code) }}" placeholder="Code">
            </div>
            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 form-group">
                ...
                <select class="form-control" id="translate_mode" name="translate_mode">
                    <option value="text"{!! old('translate_mode', $translateInfo->translate_mode) == 'text' ? ' selected="selected"' : '' !!}>Text</option>
                    <option value="editor"{!! old('translate_mode', $translateInfo->translate_mode) == 'editor' ? ' selected="selected"' : '' !!}>Editor</option>
                </select>
            </div>
            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 form-group">
                ...
                <select class="form-control" id="status" name="status">
                    @foreach (config('cms.backend.status') as $name => $value)
                        <option value="{{ $value }}"{!! $value == old('status', $translateInfo->status) ? ' selected="selected"' : '' !!}>{{ trans('common.status.' . $name) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        @foreach (config('laravellocalization.supportedLocales') as $language => $data)
            <div class="form-group has-feedback">
                <textarea class="form-control" rows="5" id="translate_content_{!! $language !!}" name="translate_content[{!! $language !!}]" placeholder="{{ $data['native'] }}" data-editor="{{ json_encode(['width' => '99%', 'height' => '150px', 'toolbarName' => 'Description', 'language' => session('backend-locale')]) }}">{{ old('translate_content.' . $language, trans_by_locale($translateInfo->translate_content, $language)) }}</textarea>
