...
<div class="sidebar-content sidebar-sm">
    <form id="frmConfig" name="frmConfig" role="form" action="{{ route('backend.configwebsite.config.update', [$configInfo->config_id]) }}" method="put">
        <div class="form-group">
            <label for="field_name" class="required">Tên cấu hình</label>
            <input type="text" class="form-control" id="field_name" name="field_name" value="{{ old('field_name', $configInfo->field_name) }}" placeholder="Tên cấu hình">
        </div>
        ...
        @foreach (config('laravellocalization.supportedLocales') as $language => $data)
            <div class="form-group has-feedback">
                <textarea class="form-control" rows="5" id="field_value_{!! $language !!}" name="field_value[{!! $language !!}]" placeholder="{{ $data['native'] }}">{{ old('field_value.' . $language, trans_by_locale($configInfo->field_value, $language)) }}</textarea>
