...
<div class="sidebar-content sidebar-sm sidebar-scroll">
    <form id="frmBlockTemplate" name="frmBlockTemplate" role="form" action="{{ route('backend.block.template.update', [$templateInfo->template_id]) }}" method="put">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="template_view" name="template_view" value="{{ old('template_view', $templateInfo->template_view) }}" placeholder="File view">
        </div>
        <div class="form-group">
            <label for="template_function">Function</label>
            <div class="checkbox">
                @foreach ($arrListFunction as $function)
                    <div><label><input type="checkbox" value="{{ $function->function_id }}" name="template_function[]"{!! in_array($function->function_id, $arrTemplateFunction) ? ' checked="checked"' : '' !!}>{{ $function->function_name }}</label></div>
                @endforeach
            </div>
        </div>
        <div class="form-group">
            <label for="template_params">Params</label>
            <input type="text" class="form-control" id="template_params" name="template_params" value="{{ old('template_params', $templateInfo->template_params) }}" placeholder="Params">
        </div>
        <div class="form-group">
