...
<div class="sidebar-content sidebar-sm">
    <form id="frmBlockPage" name="frmBlockPage" role="form" action="{{ route('backend.block.page.store') }}" method="post">
        <div class="form-group">
            <label for="page_name" class="required">Tên trang</label>
            <input type="text" class="form-control" id="page_name" name="page_name" value="{{ old('page_name') }}" placeholder="Tên trang">
        </div>
        <div class="form-group">
            <label for="page_code" class="required">Code</label>
            <input type="text" class="form-control" id="page_code" name="page_code" value="{{ old('page_code') }}" placeholder="Code">
        </div>
        <div class="form-group">
            ...
            <select class="form-control" id="page_layout" name="page_layout">
                ...
                @foreach (config('cms.backend.block.page.layout') as $layout)
                    <option value="{{ $layout }}"{!! $layout == old('page_layout') ? ' selected="selected"' : '' !!}>{{ trans('common.block.page.layout.' . $layout) }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="page_url">Url trang</label>
            <input type="text" class="form-control" id="page_url" name="page_url" value="{{ old('page_url') }}" placeholder="Url trang">
        </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="language_id" name="language_id">
                    @foreach ($arrLanguage as $lang => $data)
                        <option value="{{ $lang }}"{!! $lang == old('language_id', config('app.locale')) ? ' selected="selected"' : '' !!}>{{ $data['native'] }}</option>
                    @endforeach
                </select>
            </div>
        @else
