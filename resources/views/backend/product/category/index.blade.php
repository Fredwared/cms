...
<div class="box box-primary">
    <div class="box-header with-border">
        <form method="get" action="{{ route('backend.product.category.index') }}">
        ...
        @if (check_permission('category', 'insert'))
            <div class="text-right mb10">
                <a role="button" class="btn btn-sm btn-primary" href="{!! route('backend.product.category.create', [$language_id]) !!}"><i class="fa fa-plus"></i> {{ trans('common.action.add') }}</a>
            </div>
        @endif
    ...
    <div class="box-body table-responsive">
        @if (count($arrListCategory) > 0)
            @if (check_permission('category', 'update'))
                @foreach (config('cms.backend.status') as $name => $value)
                    <button type="button" class="btn btn-sm btn-primary" data-status="true" data-link="{{ route('backend.product.category.changestatus', [$value]) }}">{{ trans('common.status.' . $name) }}</button>
                @endforeach
                <button type="button" class="btn btn-sm btn-primary btn-show-sidebar" data-link="{!! route('backend.product.category.sort', [$language_id]) !!}">{{ trans('common.action.sort') }}</button>
            @endif
        @endif
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th>Code</th>
                    <th class="w80px text-right">Sản phẩm</th>
            ...
            @if (count($arrListCategory) > 0)
                @foreach ($arrListCategory as $category)
                    <tr>
                        ...
                        <td>
                            @if (check_permission('category', 'update'))
                                <a href="{!! route('backend.product.category.edit', [$category->category_id]) !!}">
                        ...
                        <td class="text-right">
                            <a href="{{ route('backend.product.index', ['category_id' => $category->category_id]) }}">{{ $category->products->count() }}</a>
                        </td>
                                        ...
                                        @if (empty($translation) && $language != $category->language_id)
                                            @if (check_permission('category', 'insert'))
                                                @if ($category->language_id != config('app.locale'))
                                                    <a href="{!! route('backend.product.category.create', [$language, 'item_id' => $category->category_id]) !!}" title="{{ trans('common.action.add') }}"><i class="glyphicon glyphicon-plus"></i></a>
                                                @else
                                                    <a href="{!! route('backend.product.category.create', [$language, 'source_item_id' => $category->category_id]) !!}" title="{{ trans('common.action.add') }}"><i class="glyphicon glyphicon-plus"></i></a>
                                                @endif
                                            @endif
                                        @else
                                            @if (check_permission('category', 'update'))
                                                <a href="{!! route('backend.product.category.edit', [$translation->category_id]) !!}" title="{{ trans('common.action.edit') }}"><i class="glyphicon glyphicon-edit"></i></a>
                                            @endif
                                        @endif
                                    @endforeach
                                @endif
                        ...
                        @endif
                        <td class="text-center">
                            <select class="form-control r04 wp100" data-forstatus="{{ $category->category_id }}" data-status="true" data-link="{{ route('backend.product.category.changestatus', [0]) }}" data-old="{{ $category->status }}"{!! check_permission('category', 'update') ? '' : ' disabled="disabled"' !!}>
                                @foreach (config('cms.backend.status') as $name => $value)
                        ...
                        <td class="text-center">
                            @if (check_permission('category', 'update'))
                                <a href="{!! route('backend.product.category.edit', [$category->category_id]) !!}" title="{{ trans('common.action.edit') }}"><i class="glyphicon glyphicon-edit"></i></a>
                            @endif
                            @if (check_permission('category', 'delete'))
                                <a data-delete="true" data-message="{{ trans('common.messages.category.delete') }}" href="{!! route('backend.product.category.destroy', [$category->category_id]) !!}" title="{{ trans('common.action.delete') }}"><i class="glyphicon glyphicon-trash"></i></a>
                            @endif
            ...
            @else
                <tr>
                    @if (count($arrLanguage) > 1)
                        <td colspan="9" class="text-center">{!! language_switcher() !!}</td>
                    @else
