<div class="sidebar-content sidebar-sm">
	...
	<form id="frmWard" name="frmWard" role="form" action="{{ route('backend.location.ward.store') }}" method="post">
		<div class="form-group">
			<label for="ward_name" class="required">Tên phường/xã</label>
			<input type="text" class="form-control" id="ward_name" name="ward_name" value="{{ old('ward_name') }}" placeholder="Tên phường/xã">
		</div>
		<div class="form-group">
			...
			<select class="form-control" id="district_id" name="district_id">
				...
				@foreach ($arrListDistrict as $district)
					<option value="{{ $district->district_id }}"{!! old('district_id', $districtInfo->district_id) == $district->district_id ? ' selected="selected"' : '' !!}>{{ $district->district_name }}</option>
				@endforeach
			</select>
		</div>
		<div class="form-group">
			<label for="ward_location">Vị trí</label>
			<input type="text" class="form-control" id="ward_location" name="ward_location" value="{{ old('ward_location') }}" placeholder="Vị trí">
