<div class="sidebar-content sidebar-sm">
	...
	<form id="frmCity" name="frmCity" role="form" action="{{ route('backend.location.city.store') }}" method="post">
		<div class="form-group">
			<label for="city_name" class="required">Tên thành phố/tỉnh</label>
			<input type="text" class="form-control" id="city_name" name="city_name" value="{{ old('city_name') }}" placeholder="Tên thành phố/tỉnh">
		</div>
		<div class="form-group">
			...
			<select class="form-control" id="country_id" name="country_id">
				...
				@foreach ($arrListCountry as $country)
					<option value="{{ $country->country_id }}"{!! old('country_id', $countryInfo->country_id) == $country->country_id ? ' selected="selected"' : '' !!}>{{ $country->country_name }}</option>
				@endforeach
			</select>
		</div>
        <div class="row">
            <div class="form-group col-lg-6 col-md-6 col-sm-12 col-xs-12">
                ...
                <input type="text" class="form-control" id="city_zipcode" name="city_zipcode" value="{{ old('city_zipcode') }}" placeholder="Zip code">
            </div>
            <div class="form-group col-lg-6 col-md-6 col-sm-12 col-xs-12">
                <label for="city_location">Vị trí</label>
                <input type="text" class="form-control" id="city_location" name="city_location" value="{{ old('city_location') }}" placeholder="Vị trí">
