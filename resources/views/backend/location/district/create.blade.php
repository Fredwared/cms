<div class="sidebar-content sidebar-sm">
	...
	<form id="frmDistrict" name="frmDistrict" role="form" action="{{ route('backend.location.district.store') }}" method="post">
		<div class="form-group">
			<label for="district_name" class="required">Tên quận/huyện/thị xã</label>
			<input type="text" class="form-control" id="district_name" name="district_name" value="{{ old('district_name') }}" placeholder="Tên quận/huyện/thị xã">
		</div>
		<div class="form-group">
			...
			<select class="form-control" id="city_id" name="city_id">
				...
				@foreach ($arrListCity as $city)
					<option value="{{ $city->city_id }}"{!! old('city_id', $cityInfo->city_id) == $city->city_id ? ' selected="selected"' : '' !!}>{{ $city->city_name }}</option>
				@endforeach
			</select>
		</div>
		<div class="form-group">
			<label for="district_location">Vị trí</label>
			<input type="text" class="form-control" id="district_location" name="district_location" value="{{ old('district_location') }}" placeholder="Vị trí">
