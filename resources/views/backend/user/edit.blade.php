...
<div class="box box-primary">
    <!-- form start -->
    <form id="frmUser" name="frmUser" role="form" action="{{ route('backend.user.update', [$userInfo->getAccountId()]) }}" method="post">
        <div class="box-body">
            <div class="row">
    						...
    						@if ($menu->childs->count() > 0)
    							<optgroup label="{{ trans_by_locale($menu->menu_name, session('backend-locale')) }}">
    								@foreach ($menu->childs as $child)
    									<option value="{{ $child->route_name }}"{!! $child->route_name == old('landing_page', $userInfo->landing_page) ? ' selected="selected"' : '' !!}>
                                        	{{ trans_by_locale($child->menu_name, session('backend-locale')) }}
                                    	</option>
    								@endforeach
    							</optgroup>
    						@else
    							<option value="{{ $menu->route_name }}"{!! $menu->route_name == old('landing_page', $userInfo->landing_page) ? ' selected="selected"' : '' !!}>
                ...
                @if (count(config('cms.backend.languages')) == 1)
                    <input type="hidden" id="default_language" name="default_language" value="{{ old('default_language', $userInfo->default_language) }}">
                @else
                ...
                <div class="col-lg-{!! $cols !!} col-md-{!! $cols !!} col-sm-{!! $cols !!} col-xs-6 form-group">
                    ...
                    <input type="text" class="form-control" id="days_password_expired" name="days_password_expired" value="{{ old('days_password_expired', $userInfo->days_password_expired) }}" maxlength="3" placeholder="Ngày hết hạn mật khẩu">
                </div>
                <div class="col-lg-{!! $cols !!} col-md-{!! $cols !!} col-sm-{!! $cols !!} col-xs-6 form-group">
                    <label for="status" class="required">{{ trans('common.status.title') }}</label>
                    <select class="form-control" id="status" name="status">
                    	<option value="{{ config('cms.backend.user.status.active') }}"{!! old('status', $userInfo->status) == config('cms.backend.user.status.active') ? ' selected="selected"' : '' !!}>{{ trans('common.auth.status.active') }}</option>
                        <option value="{{ config('cms.backend.user.status.inactive') }}"{!! old('status', $userInfo->status) == config('cms.backend.user.status.inactive') ? ' selected="selected"' : '' !!}>{{ trans('common.auth.status.inactive') }}</option>
                        <option value="{{ config('cms.backend.user.status.forgotpass') }}"{!! old('status', $userInfo->status) == config('cms.backend.user.status.forgotpass') ? ' selected="selected"' : '' !!}>{{ trans('common.auth.status.forgotpass') }}</option>
                        <option value="{{ config('cms.backend.user.status.banned') }}"{!! old('status', $userInfo->status) == config('cms.backend.user.status.banned') ? ' selected="selected"' : '' !!}>{{ trans('common.auth.status.banned') }}</option>
                    </select>
                </div>
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-group">
                    <label for="groups" class="required">Nhóm</label>
                    <select class="form-control" data-width="100%" data-multiselect="true" data-ajax="1" data-url="{{ route('backend.utils.search.group') }}" data-placeholder="Chọn nhóm" data-fields="group_id|group_name" id="groups" name="groups[]" multiple="multiple">
