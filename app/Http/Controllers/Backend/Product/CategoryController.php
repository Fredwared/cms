...
class CategoryController extends BackendController
{
    public function index(Request $request)
    {
        ...
        if ($arrListCategory->total() > 0) {
            ...
            if ($maxPage < $page) {
                return redirect(route('backend.product.category.index', ['item' => $item, 'page' => $maxPage]));
            }
        }
        ...
        return view('backend.product.category.index', compact('arrListCategory', 'arrLanguage', 'status', 'language_id', 'category_type', 'pagination', 'item'));
    }
    ...
    public function create(Request $request, $language = null)
    {
        ...
        if (!in_array($language, array_keys($arrLanguage))) {
            return redirect(route('backend.product.category.create', [config('app.locale')]));
        }
        ...
        return view('backend.product.category.create', compact('arrLanguage', 'language', 'category_source', 'item_id', 'category_type', 'categorySourceInfo'));
    }
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            'category_code' => 'required|regex:[^[a-z0-9\-]+$]|max:200|unique:category,category_code,null,null,category_type,' . config('cms.backend.category.type.product') . ',deleted,0,language_id,' . $request->language_id,
        ...
        return redirect(route('backend.product.category.index', ['language_id' => $request->language_id]));
    }

    public function edit(Business_Category $categoryInfo)
    {
        if ($categoryInfo->category_type == config('cms.backend.category.type.article')) {
            return redirect(route('backend.product.article.edit', [$categoryInfo->category_id]));
        }
        ...
        return view('backend.product.category.edit', compact('arrLanguage', 'categoryInfo', 'categorySource'));
    }
    ...
    public function update(Request $request, Business_Category $categoryInfo)
    {
        $this->validate($request, [
            'category_title' => 'required|max:200',
            'category_code' => 'required|regex:[^[a-z0-9\-]+$]|max:200|unique:category,category_code,' . $categoryInfo->category_id . ',category_id,category_type,' . config('cms.backend.category.type.product') . ',deleted,0,language_id,' . $request->language_id,
            'cateparent_id' => 'required',
        ...
        $params = [
            ...
            'category_icon' => $request->category_icon ?? null,
            'cateparent_id' => $request->cateparent_id,
            'language_id' => $categoryInfo->products->count() <= 0 ? $request->language_id : $categoryInfo->language_id,
            'status' => $request->status,
        ];
    ...
    public function destroy(Request $request, Business_Category $categoryInfo)
    {
        ...
        if (!$request->ajax()) {
            ...
            return redirect(route('backend.product.category.index', ['language_id' => $categoryInfo->language_id]));
        } else {
    ...
    public function changeStatus(Request $request, $status)
    {
        ...
        if (!empty($arrId)) {
            ...
            foreach ($arrId as $id) {
                $categoryInfo = $businessCategory->find($id);
                $categoryInfo->update([
                    'status' => $status
                ]);
            }
    ...
    public function sort(Request $request)
    {
        ...
        return view('backend.product.category.sort', compact('arrListCategory', 'language_id', 'category_type'));
    }
}
